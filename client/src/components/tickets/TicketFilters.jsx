...
export default function TicketFilters({ filters, onChange }) {
  ...
  const activeCount = Object.keys(filters).filter((key) => !['page', 'limit'].includes(key) && filters[key]).length;

  return (
    <div className="card p-4">
      <div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-slate-800"><SlidersHorizontal className="h-4 w-4 text-brand-600" /> Search & filters</div>{activeCount > 0 && <button onClick={() => onChange({ page: 1, limit: filters.limit || 15 })} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-800"><X className="h-3.5 w-3.5" /> Clear filters</button>}</div>
